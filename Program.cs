namespace TelegramBot {
    struct BotUpdate
    { public string text; public long id; public string? username; }
    static class Program {
        static TelegramBotClient Bot = new TelegramBotClient("5376738181:AAE2fdhQaCICZdTL7IjyQ_F-cd5qzHWHxMw");
        ...
        private static async Task UpdateHandler(ITelegramBotClient bot, Update update, CancellationToken arg3)
        {
            if (update.Type == UpdateType.Message)
            {
                if (update.Message.Type == MessageType.Text)
                {
                    ...
                    {text = update.Message.Text, id = update.Message.Chat.Id, username = update.Message.Chat.Username};
